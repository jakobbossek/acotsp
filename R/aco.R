...
aco = function(x, n.ants = 2L, alpha = 1, beta = 2, rho = 0.2) {
    #FIXME: upper should be somewhat n/2 or something of this type.
    assertInteger(n.ants, lower = 1, upper = 10)
    ...
    # get tour length
    tour.lengths = apply(ant.tours, 1, function(x) getTourLength(x, dist.mat))
    print(tour.lengths)

    best.ant = which.min(tour.lengths)
    best.tour.length = tour.lengths[best.ant]
    best.tour = ant.tours[best.ant, ]
    catf("======")
    catf("Best ant: %i", best.ant)
